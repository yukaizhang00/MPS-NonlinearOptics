...
ao = np.zeros((n,n), dtype = np.complex_)
aot = np.zeros((n,n), dtype = np.complex_)
for i in range(n-1):
    aot[(i,i+1)] += np.sqrt(i+1)
for i in range(1,n):
    ao[(i,i-1)] += np.sqrt(i)

alp = 1.

Displace = SLA.expm(alp*aot - np.conj(alp)*ao)
...
GaftV,SaftV = one_mode(GaftV, SaftV, Displace, 0)
        ...
        def intef(y):
            ...
            for j in range(n):
                for jp in range(n):
                    ...
                    phij = phij*np.power(2,1/2)
                    phijp = phijp*np.power(2,1/2)
                    ...
                    result += rou[(j,jp)]*np.exp(-1.j*np.sqrt(2) *y*p)*phij*np.conj(phijp)
            return result
...
for xi in range(len(gridx)):
    ...
    for pi in range(len(gridp)):
        p = gridp[pi]
        energyW += (np.power(x,2)+np.power(p,2))*gridW[(xi,pi)]*100/((gridn-1)*(gridn-1))
        x1spread += (np.power(x-1,2))*gridW[(xi,pi)]*100/((gridn-1)*(gridn-1))
        x2spread += (np.power(p,2))*gridW[(xi,pi)]*100/((gridn-1)*(gridn-1))
