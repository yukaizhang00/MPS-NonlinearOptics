...
#fname = 'fixed8010_0001t1000p100.p'
fname = "nbar6_chi50fixed8010_0001t1001p100.p"
with open(fname, 'rb') as f2:
    Gammas = pickle.load(f2)[1:]
    Ss = pickle.load(f2)[1:]

for iii in range(1,len(Gammas)):
    filname = os.getcwd()+'\\Pictures\\nbar'+str(nbar)+str(N)+'_d'+str(n)+ '_0001\\'+ str(iii-1) + fname.split('t')[1][:-2] + '.png'
    #filname = str(N)+'_d'+str(n)+ '_0001\\'+ str(iii-1) + fname.split('t')[1][:-2] + '.png'
    Gamma = Gammas[iii]
    S = Ss[iii]
            ...
            def intef(y):
                ...
                for j in range(n):
                    for jp in range(n):
                        ...
                        xpy = x+y/2
                        xmy = x-y/2
                        phij = (1/np.sqrt((np.power(2,j)*math.factorial(j))))*np.power(np.pi,-1/4)*np.exp(-np.power(xpy,2)/2)*np.polynomial.hermite.hermval((xpy), coefj)
                        phijp = (1/np.sqrt((np.power(2,jp)*math.factorial(jp))))*np.power(np.pi,-1/4)*np.exp(-np.power(xmy,2)/2)*np.polynomial.hermite.hermval((xmy), coefjp)
    ...
    #plt.pcolormesh(gridx, gridp, gridWr, cmap='RdBu', vmin=-0.6, vmax=0.6)
    plt.pcolormesh(gridx, gridp, np.transpose(gridWr), cmap='RdBu', vmin=-0.3, vmax=0.3)
    ...
    for xi in range(len(gridx)):
        ...
        for pi in range(len(gridp)):
            p = gridp[pi]
            energyW += 0.5*(np.power(x,2)+np.power(p,2))*gridW[(xi,pi)]*(((10+deltx)/gridn)*((10+deltx)/gridn))
    print('The energy from Wigner function',energyW)
    energies.append(energyW)
    print('The integral of Wigner function',np.sum(gridWr)*(((10+deltx)/gridn)*((10+deltx)/gridn)))
    wints.append(np.sum(gridWr)*100/((gridn-1)*(gridn-1)))
    print('The negativity of Wigner function',np.sum(np.abs(gridWr))*100/((gridn-1)*(gridn-1)) -1 )
    negativities.append(np.sum(np.abs(gridWr))*(((10+deltx)/gridn)*((10+deltx)/gridn)) -1)
    #np.einsum('jklm,jka->lma',np.tensordot(Gl, V,(0,0)),Gm)

time = [i*0.1 for i in range(len(energies))]
plt.plot(time, negativities)
plt.savefig('NegativityPlot.jpg')
plt.close()
saveoutputname = 'enerintneg' + fname
f = open(saveoutputname, 'wb')
pickle.dump(energies,f)
pickle.dump(wints,f)
pickle.dump(negativities,f)
f.close()