...
def one_mode(G, SS, V, l):
    Gcopy = [np.copy(g) for g in G]
    SScopy = [np.copy(sss) for sss in SS]
    if l != 0 and l != len(G) - 1:
        Gcopy[l] = np.einsum('ij,aib->ajb',V, G[l])
    elif l == 0:
        Gcopy[l] = np.einsum('ij,ib->jb',V, G[l])
    elif l == len(G) - 1:
        Gcopy[l] = np.einsum('ij,ai->aj',V, G[l])
    return Gcopy, SScopy
...
def two_mode(G, SS, V, l, err = 1e-16, normalize = False):
    ...
    if l != 0 and l != len(G)-2:
        ...
        Theta = np.tensordot(Theta, np.diag(SS[l+1]),(-1,0))
        Theta = np.tensordot(np.diag(SS[l-1]), Theta,(-1,0))
        Tshape = Theta.shape
        ...
        for alpha in range(len(SS[l-1])):
            Gl_new[alpha] = Gl_new[alpha]/SS[l-1][alpha]
        for alpha in range(len(SS[l+1])):
            Gm_new[:,:,alpha] = Gm_new[:,:,alpha]/SS[l+1][alpha]
        '''
        ...
        '''
        #Cutoff at 40 for all
        if len(s) > ccnum:
            reind = np.argsort(-s)[:ccnum]
        else:
            reind = np.argsort(-s)
        tot = np.sqrt(np.sum(np.conj(s)*s))
        s = s[reind]*tot/np.sqrt(np.sum(np.conj(s[reind])*s[reind]))
        u = np.transpose(np.transpose(u)[reind])
        v = v[reind]
        
        #Cutoff for small s
        reind = []
        for ii in range(len(s)):
            if np.abs(s[ii]) > cutoff:
                reind.append(ii)
        #print(s.shape,'guagua')
        #print(reind)
        s = s[reind]*tot/np.sqrt(np.sum(np.conj(s[reind])*s[reind]))
        u = np.transpose(np.transpose(u)[reind])
        v = v[reind]
...
def two_mode_o(G, SS, V, l, err = 1e-16, normalize = False):
    #The function input G is Gamma tensors and SS is Lambdas
    #V is two mode operator, l is location of V at l and l+1 (0 <= l <= N-2)
    #The function will return new Gammas and Lambdas
    Gcopy = G
...
def two_mode_s(G, SS, V, l, err = 1e-16, ccnum = 50):
    #SS = [<lambda_l>, ?<lambda_l-1>, ?<lambda_l+1>]
    Gcopy = G
    SScopy = SS
    Gl = G[0]
    cutoff = 1e-9
    #Here SS[l+1] or SS{l]
    Gm = np.tensordot(np.diag(SS[0]), G[1], (0,0))
    Tbefore = np.tensordot(Gl,Gm,(-1,0))
    #print('The original norm is', np.sqrt(np.sum(np.conj(Tbefore)*Tbefore)))
    if l != 0 and l != len(G)-2:
        #Calculate Theta^ij_alphagamma with natural index order [alpha, i, j, gamma]:
        Theta = np.einsum('ijklm,jka->ilma',np.tensordot(Gl, V,(1,0)),Gm)
        Theta = np.tensordot(Theta, np.diag(SS[2]),(-1,0))
        Theta = np.tensordot(np.diag(SS[1]), Theta,(-1,0))
        Tshape = Theta.shape
        #Mtheta is matrix form with index (alpha, i) x (j, gamma)
        Mtheta = Theta.reshape((Tshape[0]*Tshape[1], Tshape[2]*Tshape[3]))
        u,s,v = LA.svd(Mtheta, full_matrices=False)
        ...
        for alpha in range(len(SS[1])):
            Gl_new[alpha] = Gl_new[alpha]/SS[1][alpha]
        for alpha in range(len(SS[2])):
            Gm_new[:,:,alpha] = Gm_new[:,:,alpha]/SS[2][alpha]
        #Cutoff at 40 for all
        if len(s) > ccnum:
            reind = np.argsort(-s)[:ccnum]
        else:
        ...
        Gl_new = u.reshape(Tshape[0], Tshape[1], len(s))
        Gm_new = v.reshape(len(s), Tshape[2], Tshape[3])
        return [Gl_new,Gm_new] , s
    elif l == 0:
        Theta = np.einsum('jklm,jka->lma',np.tensordot(Gl, V,(0,0)),Gm)
        Theta = np.tensordot(Theta, np.diag(SS[1]),(-1,0))
        Tshape = Theta.shape
        #Mtheta is matrix form with index (alpha, i) x (j, gamma)
        Mtheta = Theta.reshape((Tshape[0], Tshape[1]*Tshape[2]))
        u,s,v = LA.svd(Mtheta, full_matrices=False)
        ...
        recrTheta = np.tensordot(np.tensordot(Gl_new, np.diag(s),(1,0)), Gm_new, (1,0))
        for alpha in range(len(SS[1])):
            Gm_new[:,:,alpha] = Gm_new[:,:,alpha]/SS[1][alpha]
        return [Gl_new, Gm_new], s
        ...
        recrTheta = np.tensordot(np.tensordot(Gl_new, np.diag(s),(2,0)), Gm_new, (2,0))
        for j in range(len(s)):
            temp2 = np.tensordot(np.diag(SS[1]),Gl_new,(0,0))[...,j]
            coefGl = np.sqrt(np.sum(np.conj(temp2)*temp2))
            Gl_new[...,j] = Gl_new[...,j]/coefGl
            s[j] = s[j]*coefGl.real
        return [Gl_new, Gm_new], s